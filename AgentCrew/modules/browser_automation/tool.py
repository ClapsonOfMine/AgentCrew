...
Provides four tools for browser automation:
- browser_navigate: Navigate to URLs
- browser_click: Click elements using XPath selectors  
- browser_scroll: Scroll page content in specified directions
- browser_get_content: Extract page content and clickable elements as markdown
"""
...
def get_browser_get_content_tool_definition(provider="claude") -> Dict[str, Any]:
    ...
    Returns:
        Dict containing the tool definition
    """
    tool_description = "Extract the current page content and identify all clickable elements. Returns the page content converted to markdown format along with a table of clickable elements with their XPath selectors. Use this to understand what's currently visible on the page and to identify elements you can interact with using browser_click."
    tool_arguments = {}
    tool_required = []
...
def get_browser_navigate_tool_handler(browser_service: BrowserAutomationService) -> Callable:
    """
...
def get_browser_click_tool_handler(browser_service: BrowserAutomationService) -> Callable:
    """
...
def get_browser_scroll_tool_handler(browser_service: BrowserAutomationService) -> Callable:
    """
    ...
    def handle_browser_scroll(**params) -> str:
        ...
        if direction not in ["up", "down", "left", "right"]:
            return "Error: Invalid scroll direction. Use 'up', 'down', 'left', or 'right'."
...
def get_browser_get_content_tool_handler(browser_service: BrowserAutomationService) -> Callable:
    """
    ...
    def handle_browser_get_content(**params) -> str:
        ...
        if result["success"]:
            content_info = f"📄 Page content extracted successfully\n"
            content_info += f"URL: {result.get('url', 'Unknown')}\n"
            content_info += f"Content length: {result.get('content_length', 0)} characters\n"
            
            if result.get('has_clickable_elements'):
                content_info += "✅ Clickable elements found and listed below\n\n"
            else:
                content_info += "ℹ️ No clickable elements found on this page\n\n"
...
def register(service_instance=None, agent=None):
    ...
    # Register all four browser automation tools
    register_tool(
